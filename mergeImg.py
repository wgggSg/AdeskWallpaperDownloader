...
def resize_by_width(infile, image_size):
    ...
    y_s = int(y // lv)
    print("x_s", x_s, y_s)
    out = infile.resize((x_s, y_s), Image.ANTIALIAS)
    return out
...
# 定义图像拼接函数
def image_compose(image_colnum, image_size, image_rownum, images, x_new, y_new):
    ...
    for y in range(1, image_rownum + 1):
        for x in range(1, image_colnum + 1):
            from_image = resize_by_width(images[image_colnum * (y - 1) + x - 1], image_size)
...
def merge_images(images,image_size,image_colnum):
    # image_rownum = 4  # 图片间隔，也就是合并成一张图后，一共有几行
    image_rownum_yu = len(images) % image_colnum
    if image_rownum_yu == 0:
        image_rownum = len(images) // image_colnum
    else:
        image_rownum = len(images) // image_colnum + 1
    ...
    return image_compose(image_colnum, image_size, image_rownum, images, x_new, y_new)  # 调用函数
